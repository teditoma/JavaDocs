...
public class MyList<Integer> extends ArrayList<Integer> {
    ...
    private void checkDifferent( Integer integer) {
        boolean different = true;
        for ( Integer i: super.subList(0,super.size()))
            if (integer == i)
                different = false;
        if (different) {
            differentElements++;
    ...
    private void removeDifferent(Integer integer){
        int different = 0;
        for (Integer i: super.subList(0,super.size()))
            if ( i == integer)
                different ++;
    ...
    public Integer remove(int index) {
        removeDifferent(super.get(index));
        //list.remove(index);
